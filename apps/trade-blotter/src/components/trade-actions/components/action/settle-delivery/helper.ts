...
export const settleDeliveryTrades = (
  ...
  console.log('Settling trades ', serviceFormat);
  if (str.isNotEmpty(serviceFormat.batchUuid)) {
    return performActionOnMultipleTrade(requestData, user, serviceFormat).then((response) => {
      console.log('Settle Delivery response for bulk trade', response);
      if (response.ok) {
    ...
    return performActionOnSingleTrade(requestData, user, serviceFormat).then((response) => {
      console.log('Settle Delivery response for single trade', response);
      if (response.ok) {
